...
def get_router_info(host, apikey, verify=False):
    ...
    try:
        ...
        # NEW: Get version from 'show version' command
        try:
            version_data = api_retrieve(host, apikey, verify, op='showVersion')
            version_str = version_data.get('version', '')
            
            # LOGIC: If it's 1.4 or 1.3, use 1.4 syntax. Everything else uses 1.5 syntax.
            if '1.4' in version_str or '1.3' in version_str:
                info['version'] = '1.4'
            else:
                # Default to 1.5 for: 1.5-rolling, 2025.x, 2026.x, future versions
                info['version'] = '1.5'
        except:
            # Fallback: check config structure
            info['version'] = None
        
        # Get full config
        config = api_retrieve(host, apikey, verify)
        
        # FALLBACK: If version detection failed, use config-based detection
        if info['version'] is None:
            # Look for 1.4-specific markers
            if 'traffic-policy' in config:
                info['version'] = '1.4'
            # Check firewall structure
            elif 'firewall' in config and 'name' in config['firewall'] and 'ipv4' not in config['firewall']:
                info['version'] = '1.4'
            else:
                # Default to modern syntax (1.5+)
                info['version'] = '1.5'
