...
## [1.1.2-patch.33.39] - 2026-02-01
### Changed
- **CI/CD Optimization**: Disabled automatic multi-platform builds for version tags to prioritize release speed. Full builds (AMD64 + ARM64) now only trigger when explicitly requested via manual GitHub Actions execution. 🐳⚡

## [1.1.2-patch.33.38] - 2026-02-01
### Changed
...
- **Non-Interactive Installation**: Refactored `install.sh` and `install_latest.sh` to be non-interactive by default. 🚀
    - Removed `/dev/tty` redirection.
    - Defaulted to "Full Dashboard" mode (can be overridden with `--target` flag).
    - Preserved upgrade/overwrite interactivity for existing installations.
...
- **Interactive Install Fix**: Added `stdin` redirection to `/dev/tty` in `install.sh` and `install_latest.sh`. This fixes a bug where interactive prompts were skipped when the script was piped from `curl` to `bash`. 🛠️🏎️
...
### Fixed
- **Web UI**: Fixed a container crash (`ERR_MODULE_NOT_FOUND`) by adding the missing `iot-manager.ts` file to the Docker build.
- **Voice Simulation**: Fixed a bug where simulation parameters (Max Simultaneous Calls, etc.) would automatically revert to their previous values while editing due to rapid background polling.
