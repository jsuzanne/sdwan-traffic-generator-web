                ...
                try:
                    ...
                    if debug:
                        print(
                            ...
                            f"{intf_name} used_for=lan -> {ip_address} ({network})",
                            file=sys.stderr
                        )
...
def get_all_dc_lan_interfaces(api_client, sites, debug=False):
    """
    DC LAN interfaces:
    - used_for == 'private'
    - ipv4_config.type == 'static'
    - pas de circuit attaché (label_id, circuit_label_id, wan_network_id)
    """
    site_lan_map = {}
    ...
    try:
        ...
        elements_resp = api_client.get.elements()
        if not elements_resp.cgx_status:
            if debug:
                print(f" Could not get elements: {elements_resp.status_code}", file=sys.stderr)
            return {}
    ...
    for element in all_elements:
        ...
        try:
            intf_resp = api_client.get.interfaces(site_id=element_site_id, element_id=element.get('id'))
            if not intf_resp.cgx_status:
                if debug:
                    print(f" Could not get interfaces: {intf_resp.status_code}", file=sys.stderr)
                continue
            ...
            for interface in interfaces:
                ...
                # Filtre 2: exclure les interfaces liées à un WAN interface (overlay)
                site_wan_interface_ids = interface.get('site_wan_interface_ids')
                if site_wan_interface_ids:
                    if debug:
                        print(
                            f" [SKIP] {site_name}/{element_name}/{intf_name} linked to WAN interface {site_wan_interface_ids}",
                            file=sys.stderr
                        )
                    continue

                # Filtre 3: exclure les interfaces avec circuit attaché
                circuit = (
                ...
                if circuit:
                    if debug:
                        print(
                            f" [SKIP] {site_name}/{element_name}/{intf_name} has circuit {circuit}",
                            file=sys.stderr
                        )
                    continue
                ...
                # # DEBUG: dump les interfaces 192.168.190.x pour voir le champ circuit
                # if debug and ip_address_raw and "192.168.190" in ip_address_raw:
                #     print(f"\n [DEBUG-190] Interface {site_name}/{element_name}/{intf_name}:", file=sys.stderr)
                ...
                try:
                    ...
                    if debug:
                        print(
                            ...
                            f"{intf_name} used_for=private -> {ip_address} ({network})",
                            file=sys.stderr
                        )
            ...
            if not flat:
                ...
                for item in flat:
                    log_output(
                        ...
                        f"{item['interface_name']}\t{item['ip']}\t{item['network']}",
                        json_mode
                    )
